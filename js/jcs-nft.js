...
(function (root) {
  ...
  async function acceptReceipt(detail, retryPending = false) {
    if (!HASH.test(detail?.txid || '') || (detail.kind !== undefined && !KIND[detail.kind]) || signing) return;
  ...
  async function prepareMint() {
    ...
    finally { if (token === version && sequence === preparationSequence && byId('jcsReceiptMint').textContent !== 'Receipt mint confirmed') byId('jcsReceiptMint').disabled = false; }
  }
